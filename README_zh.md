...
> [AirymaxOS](https://atomgit.com/openairymax/agentrt-linux)（智能体操作系统）的服务子系统。
...
**AirymaxOS 服务态**（`airymaxos-services`）是 AirymaxOS（智能体操作系统）的用户态服务子系统。它实现微内核用户态服务层：将 VFS、网络与驱动从内核移至用户态守护进程，通过 12 个专用守护进程与 systemd 集成，并经 io_uring 消息传递与内核通信。
...
- **systemd 集成** 对齐 openEuler 服务管理标准
- **基于 capability 的服务访问** 委托给 AirymaxOS 安全态子系统

### 与 Airymax daemons 的关系

AirymaxOS 服务态复用并扩展了 Airymax 运行时平台的 `daemons` 模块。守护进程框架、IPC 契约与监督模型在用户态运行时（agentrt）与 OS 级服务层（AirymaxOS）之间共享，确保架构同源、无适配层。
...
- **AirymaxOS 内核** — 提供微内核原语、sched_ext 与 io_uring 入口
- **Airymax daemons** — 提供被复用并扩展的守护进程框架与 IPC 契约
- **openEuler 24.03 LTS / 26.03** — systemd 与服务标准参考发行版

### 下游

- **AirymaxOS 系统态** — 对服务集合进行打包与配置的系统层
- **AirymaxOS Cloudnative** — 消费服务面的云原生层
